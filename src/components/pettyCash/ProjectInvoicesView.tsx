...
export const ProjectInvoicesView: React.FC<ProjectInvoicesViewProps> = ({
  ...
  const filteredInvoices = useMemo(() => {
    return projectInvoices.filter(inv => {
      ...
      if (searchQuery.trim()) {
        ...
        const desc = (inv.billingDescription || inv.REMARKS || '').toLowerCase();
        const proj = (inv.PROJECT || '').toLowerCase();
        if (!num.includes(q) && !client.includes(q) && !desc.includes(q) && !proj.includes(q)) {
          return false;
        }
      }
  ...
  }, [projectInvoices, selectedProject, statusFilter, searchQuery, dateRange]);
  ...
  return (
    <div className="space-y-5 pb-14 font-sans">
        ...
        <div className="flex items-center gap-2">
          ...
          {(userRole === 'ADMIN' || userRole === 'FINANCE') && (
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold shadow-lg shadow-indigo-600/20 transition-all active:scale-95"
            >
      ...
      {activeSubTab === 'invoices-list' ? (
        <>
          {/* Filter Bar */}
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-3.5 sm:p-4 shadow-md flex flex-wrap items-center justify-between gap-3">
            {/* Search Input */}
            <div className="relative flex-1 min-w-[220px]">
              ...
              <input
                ...
                type="text"
                placeholder="Search by invoice #, client, description, project..."
                value={searchQuery}
            ...
            <div className="flex items-center gap-1.5">
              ...
              <select
                ...
                className="bg-slate-950 border border-slate-700 rounded-xl px-3 py-2 text-xs font-semibold text-slate-100 focus:outline-none focus:border-indigo-500"
              >
                  ...
                  {filteredInvoices.length === 0 ? (
                    <tr>
                      <td colSpan={12} className="py-12 text-center text-slate-500 font-sans">
                        ...
                        <p className="font-semibold text-slate-400 text-sm">No Invoices Found</p>
                        ...
                        {(userRole === 'ADMIN' || userRole === 'FINANCE') && (
                          <button
                            ...
                            className="mt-3 px-4 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold inline-flex items-center gap-1.5 shadow-md"
                          >
                            <PlusCircle className="w-3.5 h-3.5" />
                            <span>Create First Invoice for {selectedProject !== 'ALL' ? selectedProject : 'Project'}</span>
                          </button>
                        )}
                      </td>
                    </tr>
