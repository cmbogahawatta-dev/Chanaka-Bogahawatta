...
interface BulkImportIncomeModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: (batchId: string) => void;
}

export const BulkImportIncomeModal: React.FC<BulkImportIncomeModalProps> = ({
  isOpen,
  onClose,
  onSuccess
}) => {
  ...
  const [approverName, setApproverName] = useState<string>(
    userRole === 'ADMIN' ? 'Head Office Chief Accountant' : 'Petty Cash Treasury Manager'
  );
  const [approvalRemarks, setApprovalRemarks] = useState<string>('Authorized petty cash float top-ups and cash receipts bulk batch');
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      setParseError(err.message || 'Failed to parse selected income / top-up file.');
    } finally {
      setIsParsing(false);
    }
  };
  ...
  const handleExecuteImport = async () => {
    ...
    const batchId = `IMP-INC-${Date.now().toString().slice(-6)}`;

    try {
      const result = bulkImportIncome(batchId, validationSummary, {
      ...
      setAdminPinError(err.message || 'Income bulk import execution failed.');
    }
  };
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-5xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-xl font-bold text-slate-100">Bulk Import Income / Top-up Records</h2>
              ...
              <p className="text-xs text-slate-400">
                Import cash float top-ups, bank transfers to custodians, and direct site credits with automatic balance recalculation
              </p>
            </div>
          </div>
          <button
        ...
        <div className="px-6 py-3 bg-slate-950/60 border-b border-slate-800/80">
          <div className="flex items-center justify-between max-w-3xl mx-auto">
            <div className={`flex items-center gap-2 text-xs font-semibold ${currentStep >= 1 ? 'text-emerald-400' : 'text-slate-500'}`}>
              <div className={`w-6 h-6 rounded-full flex items-center justify-center border text-xs ${
                currentStep > 1 ? 'bg-emerald-500 text-white border-emerald-400' : currentStep === 1 ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500' : 'border-slate-700 bg-slate-800'
              }`}>
            ...
            <div className={`h-0.5 w-12 ${currentStep >= 2 ? 'bg-emerald-500' : 'bg-slate-800'}`} />

            <div className={`flex items-center gap-2 text-xs font-semibold ${currentStep >= 2 ? 'text-emerald-400' : 'text-slate-500'}`}>
              <div className={`w-6 h-6 rounded-full flex items-center justify-center border text-xs ${
                currentStep > 2 ? 'bg-emerald-500 text-white border-emerald-400' : currentStep === 2 ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500' : 'border-slate-700 bg-slate-800'
              }`}>
            ...
            <div className={`h-0.5 w-12 ${currentStep >= 3 ? 'bg-emerald-400' : 'text-slate-500'}`} />

            <div className={`flex items-center gap-2 text-xs font-semibold ${currentStep >= 3 ? 'text-emerald-400' : 'text-slate-500'}`}>
              <div className={`w-6 h-6 rounded-full flex items-center justify-center border text-xs ${
                currentStep > 3 ? 'bg-emerald-500 text-white border-emerald-400' : currentStep === 3 ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500' : 'border-slate-700 bg-slate-800'
              }`}>
        ...
        <div className="p-6 flex-1 overflow-y-auto">
              ...
              {inputTab === 'upload' ? (
                ...
                >
                  ...
                  <div>
                    <p className="text-sm font-semibold text-slate-200">
                      Click to choose or drag & drop Income & Top-up spreadsheet
                    </p>
                  ...
                  <div className="flex items-center gap-4 text-xs text-slate-500 mt-2">
                    <span>Fields: Date, Supervisor, Project, Amount, Income Source, Remarks</span>
                    <span>•</span>
                ...
                <div className="space-y-3">
                  <textarea
                    ...
                    placeholder="Paste tabular income/top-up rows copied directly from Excel, Google Sheets, or CSV file here..."
                    className="w-full bg-slate-950 border border-slate-700 rounded-xl p-4 text-xs font-mono text-slate-200 placeholder-slate-500 focus:outline-none focus:border-emerald-500"
                  />
                  <div className="flex justify-end">
                    <button
                      onClick={handlePasteProcess}
                      disabled={isParsing || !pastedText.trim()}
                      className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white rounded-xl text-xs font-semibold flex items-center gap-2 transition"
                    >
                      {isParsing ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <ArrowRight className="w-3.5 h-3.5" />}
                      Parse Pasted Income Records
                    </button>
                  </div>
                </div>
              )}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-4 flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">
                      {validationSummary?.totalRows} Total Records • {parsedData?.headers.length} Source Columns
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {validationSummary && validationSummary.duplicatesCount > 0 && (
                    <span className="px-3 py-1.5 bg-purple-500/10 text-purple-400 border border-purple-500/20 rounded-lg text-xs font-medium flex items-center gap-1.5">
                      ...
                      {validationSummary.duplicatesCount} Duplicates
                    </span>
                  )}
              ...
              <div className="bg-slate-950/40 border border-slate-800 rounded-xl p-4 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-slate-400">Match file columns to Income / Float Top-up schema</span>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-slate-300 uppercase tracking-wider">
                      Income Validation Grid Preview
                    </span>
                  ...
                  <table className="w-full text-left text-xs text-slate-300">
                    <thead className="bg-slate-950/80 text-slate-400 sticky top-0 border-b border-slate-800">
                      <tr>
                        ...
                        <th className="py-2 px-3 font-semibold">Supervisor</th>
                        <th className="py-2 px-3 font-semibold">Project</th>
                        <th className="py-2 px-3 font-semibold">Amount (LKR)</th>
                      ...
                      {filteredRows.map((row) => (
                        <tr key={row.rowIndex} className="hover:bg-slate-800/30 transition">
                          ...
                          <td className="py-2 px-3 font-bold text-amber-300">
                            {row.mapped['SUPERVISOR'] || '<Missing>'}
                          </td>
                          <td className="py-2 px-3 font-bold text-blue-300">
                            {row.mapped['PROJECT'] || '<Missing>'}
                          </td>
                          <td className="py-2 px-3 font-bold text-emerald-400">
                            {formatLKR(Number(row.mapped['AMOUNT']) || 0)}
                          </td>
                          <td className="py-2 px-3 text-slate-300 font-sans text-[11px]">
                            {row.mapped['INCOME_SOURCE'] || 'Float Top-up'}
                          </td>
              ...
              <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl flex items-start gap-3 text-amber-300 text-xs">
                ...
                <div>
                  ...
                  <p className="mt-1 text-slate-300">
                    Bulk crediting income directly increases petty cash balances in hand for site supervisors and reconciles against corporate bank disbursals.
                  </p>
                </div>
              </div>

              <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-5 space-y-4">
                <h4 className="text-sm font-semibold text-slate-200">Import Policies & Master Settings</h4>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-semibold text-slate-300 block mb-1.5">
                      Duplicate Income ID Policy
                    </label>
                    <select
                    ...
                    >
                      <option value="SKIP">Skip Matching Duplicate Income IDs</option>
                      <option value="IMPORT_AS_NEW">Import With New Generated Income ID</option>
                      <option value="UPDATE">Update Existing Income Record Amount</option>
                  ...
                  <div className="space-y-2 pt-1">
                    <label className="text-xs font-semibold text-slate-300 block">
                      Auto-Provision Missing Entities
                    </label>
                    <label className="flex items-center gap-2 text-xs text-slate-300 cursor-pointer">
                      ...
                      />
                      Auto-register new Supervisors found in file
                    </label>
                    <label className="flex items-center gap-2 text-xs text-slate-300 cursor-pointer">
                      <input
                        type="checkbox"
                        ...
                        className="rounded border-slate-700 text-emerald-600 focus:ring-emerald-500"
                      />
                      Auto-register new Projects found in file
                    </label>
                  </div>
                </div>
                ...
                <div>
                  <label className="text-xs font-semibold text-slate-300 block mb-1.5">
                    Authorizing Finance Officer / Chief Accountant
                  </label>
                  <input
                    type="text"
                    value={approverName}
                    ...
                    className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none focus:border-emerald-500"
                    placeholder="Enter authorizing finance officer's name"
                  />
                </div>

                <div>
                  <label className="text-xs font-semibold text-slate-300 block mb-1.5">
                    Batch Remarks / Treasury Audit Note
                  </label>
                  <textarea
                    rows={2}
                    ...
                    className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none focus:border-emerald-500"
                    placeholder="Optional administrative notes for this income/top-up bulk batch"
                  />
                </div>
          ...
          {currentStep === 4 && importResult && (
            <div className="space-y-6 max-w-md mx-auto text-center py-6">
              ...
              <div>
                <h3 className="text-xl font-bold text-slate-100">Income Records Imported Successfully!</h3>
                <p className="text-xs text-slate-400 mt-1">
                  Batch Reference: <span className="font-mono text-emerald-400 font-semibold">{importResult.batchId}</span>
                </p>
              </div>

              <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-4 text-left space-y-3">
                <div className="flex justify-between text-xs">
                  <span className="text-slate-400">Total Top-up Records:</span>
                  <span className="text-slate-200 font-bold">{importResult.count} Transactions</span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-slate-400">Total Cash Value Credited:</span>
                  <span className="text-emerald-400 font-bold font-mono">{formatLKR(importResult.totalAmount)}</span>
                </div>
                <div className="flex justify-between text-xs">
              ...
              <div className="flex gap-3 justify-center">
                <button
                  onClick={onClose}
                  className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-semibold transition shadow-lg shadow-emerald-950"
                >
                  Close & View Income Registry
                </button>
              </div>
            </div>
          )}
          ...
          <div className="flex items-center gap-3">
            ...
            {currentStep === 2 && (
              <button
                ...
                className="px-5 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white rounded-xl text-xs font-semibold flex items-center gap-1.5 transition"
              >
            ...
            {currentStep === 3 && (
              ...
              >
                <ShieldCheck className="w-4 h-4" /> Authorize & Credit Income Records
              </button>
            )}
          </div>
        </div>
