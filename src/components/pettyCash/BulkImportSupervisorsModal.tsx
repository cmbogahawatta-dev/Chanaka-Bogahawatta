...
interface BulkImportSupervisorsModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: (batchId: string) => void;
}

export const BulkImportSupervisorsModal: React.FC<BulkImportSupervisorsModalProps> = ({
  isOpen,
  onClose,
  onSuccess
}) => {
  ...
  const [approverName, setApproverName] = useState<string>(
    userRole === 'ADMIN' ? 'Site Administration Manager' : 'HR & Operations Lead'
  );
  const [approvalRemarks, setApprovalRemarks] = useState<string>('Authorized supervisor directory bulk migration batch');
  ...
  const processParsedData = (data: ParsedRawData) => {
    setParsedData(data);
    const suggested = DataImportService.autoMapColumns('SUPERVISOR_DIRECTORY', data.headers);
  ...
  const handleExecuteImport = async () => {
    ...
    const batchId = `IMP-SUP-${Date.now().toString().slice(-6)}`;

    try {
      const result = bulkImportSupervisors(batchId, validationSummary, {
      ...
      setAdminPinError(err.message || 'Import execution failed.');
    }
  };
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-5xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-xl font-bold text-slate-100">Bulk Import Supervisor Directory</h2>
              ...
              <p className="text-xs text-slate-400">
                Migrate, register, and update site supervisors & initial cash float limits from Excel/CSV
              </p>
            </div>
          </div>
          <button
        ...
        {!isAuthorizedFinancialRole ? (
          <div className="p-12 text-center space-y-4 my-auto">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-100">Financial Administrative Authorization Required</h3>
              <p className="text-xs text-slate-400 max-w-md mx-auto mt-1">
                Bulk importing site supervisor records, setting employee identities, and modifying petty cash float balance limits is strictly restricted to authorized roles (ADMIN, FINANCE, OWNER).
              </p>
            </div>
            <button
              onClick={onClose}
              className="px-5 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-xl text-xs font-bold transition shadow-md"
            >
              Close
            </button>
          </div>
          ...
          {currentStep === 1 && (
            <div className="space-y-6 max-w-3xl mx-auto">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-base font-semibold text-slate-100">Upload Supervisor Records File</h3>
                  <p className="text-xs text-slate-400">Select an Excel (.xlsx / .xls) or CSV sheet, or paste tabulated text.</p>
                </div>
              ...
              {inputTab === 'upload' ? (
                ...
                >
                  ...
                  <div>
                    <p className="text-sm font-semibold text-slate-200">
                      Click to choose or drag & drop supervisor master spreadsheet
                    </p>
                  ...
                  <div className="flex items-center gap-4 text-xs text-slate-500 mt-2">
                    <span>Includes: Full Name, Phone, Email, Float, Project</span>
                    <span>•</span>
                ...
                <div className="space-y-3">
                  <textarea
                    ...
                    placeholder="Paste tabular supervisor rows copied directly from Excel, Google Sheets, or CSV file here..."
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-4 flex flex-wrap items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">
                      {validationSummary?.totalRows} Total Rows • {parsedData?.headers.length} Source Columns
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {validationSummary && validationSummary.duplicatesCount > 0 && (
                    <span className="px-3 py-1.5 bg-purple-500/10 text-purple-400 border border-purple-500/20 rounded-lg text-xs font-medium flex items-center gap-1.5">
                      ...
                      {validationSummary.duplicatesCount} Existing Supervisors
                    </span>
                  )}
              ...
              <div className="bg-slate-950/40 border border-slate-800 rounded-xl p-4 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-slate-400">Match file columns to Supervisor schema</span>
                </div>
                  ...
                  <table className="w-full text-left text-xs text-slate-300">
                    <thead className="bg-slate-950/80 text-slate-400 sticky top-0 border-b border-slate-800">
                      <tr>
                        ...
                        <th className="py-2 px-3 font-semibold">Supervisor Name</th>
                        <th className="py-2 px-3 font-semibold">Phone Contact</th>
                        <th className="py-2 px-3 font-semibold">Opening Float (LKR)</th>
                        <th className="py-2 px-3 font-semibold">Default Project</th>
                      ...
                      {filteredRows.map((row) => (
                        <tr key={row.rowIndex} className="hover:bg-slate-800/30 transition">
                          ...
                          <td className="py-2 px-3 font-bold text-slate-200">
                            {row.mapped['SUPERVISOR_NAME'] || '<Missing>'}
                          </td>
                          <td className="py-2 px-3 text-slate-400">
                            {row.mapped['PHONE'] || '—'}
                          </td>
                          <td className="py-2 px-3 text-emerald-400">
                            {formatLKR(Number(row.mapped['OPENING_PETTY_CASH']) || 0)}
                          </td>
                          <td className="py-2 px-3 text-blue-300">
                            {row.mapped['DEFAULT_PROJECT'] || 'PIDM 26'}
                          </td>
                          <td className="py-2 px-3 text-slate-400">
                            {row.errors.length > 0 && (
              ...
              <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl flex items-start gap-3 text-amber-300 text-xs">
                ...
                <div>
                  ...
                  <p className="mt-1 text-slate-300">
                    Bulk modifying or adding supervisor records changes float allocations, project authorizations, and authentication credentials.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-5 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    >
                      <option value="UPDATE">Update Existing Supervisor Data & Balance</option>
                      <option value="SKIP">Skip Existing Supervisors</option>
                      <option value="IMPORT_AS_NEW">Import with Generated New ID</option>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-4 pt-1.5">
                      <label className="flex items-center gap-2 text-xs text-slate-300 cursor-pointer">
                        <input
                          type="radio"
                          checked={defaultActiveStatus}
                          onChange={() => setDefaultActiveStatus(true)}
                      ...
                      <label className="flex items-center gap-2 text-xs text-slate-300 cursor-pointer">
                        <input
                          type="radio"
                          checked={!defaultActiveStatus}
                          onChange={() => setDefaultActiveStatus(false)}
                          className="text-blue-600 focus:ring-blue-500"
                        />
                        Inactive / On Hold
                      </label>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none focus:border-blue-500"
                    placeholder="Optional administrative notes for this supervisor bulk batch"
                  />
                </div>
          ...
          {currentStep === 4 && importResult && (
            <div className="space-y-6 max-w-md mx-auto text-center py-6">
              ...
              <div>
                <h3 className="text-xl font-bold text-slate-100">Supervisors Imported Successfully!</h3>
                <p className="text-xs text-slate-400 mt-1">
              ...
              <div className="bg-slate-950/60 border border-slate-800 rounded-xl p-4 text-left space-y-3">
                <div className="flex justify-between text-xs">
                  <span className="text-slate-400">Total Processed Supervisors:</span>
                  <span className="text-slate-200 font-bold">{importResult.count} Supervisors</span>
                </div>
                <div className="flex justify-between text-xs">
              ...
              <div className="flex gap-3 justify-center">
                ...
                >
                  Close & View Supervisor Directory
                </button>
              </div>
            </div>
          )}
        ...
        <div className="px-6 py-4 border-t border-slate-800 bg-slate-900/80 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {currentStep === 3 && (
              ...
              >
                <ShieldCheck className="w-4 h-4" /> Authorize & Import Supervisors
              </button>
            )}
          </div>
        </div>
